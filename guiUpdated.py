...
def translated ():
    ...
    global from_language_key, to_from_language_key
    trans_box.delete(1.0, END)
...
def text_to_speech():
   # textToSpeech.speak(txt_box.get("1.0", END), detect(txt_box.get("1.0", END)))
    pygame.mixer.music.load(textToSpeech.speak(txt_box.get("1.0", END), detect(txt_box.get("1.0", END))), 'mp3')
    # pygame.mixer.music.load(textToSpeech.speak(txt_box.get("1.0", END), detect(combo_1.get())), 'mp3')
    pygame.mixer.music.play()
    # wait for audio to be played
    time.sleep(2)

def text_to_speech_1():
    pygame.mixer.music.load(textToSpeech.speak(trans_box.get("1.0", END), detect(trans_box.get("1.0", END))), 'mp3')
    # pygame.mixer.music.load(textToSpeech.speak(trans_box.get("1.0", END), detect(combo_trans.get())), 'mp3')
    pygame.mixer.music.play()
    # wait for audio to be played
    time.sleep(2)
...
# Text box input
txt_box = Text(root, height=10, width=40, font='Arial')
...
trans_box = Text(root, height=10, width=40, font='Arial')
...
button_tts2 = Button(root,
                     ...
                     fg="#e0e0e0",
                     font=("Helvetica", 15),
                     command=text_to_speech_1)
button_tts2.grid(row=2, column=2, pady=10, padx=5)

button_stt_1 = Button(root,
                      text="Speech to text",
                      bg="#46474f",
                      fg="#e0e0e0",
                      font=("Helvetica", 15),
                      command=speech_to_text)
button_stt_1.grid(row=3, column=0, pady=10, padx=5)
...
button_stt_1 = Button(root,
                      ...
                      font=("Helvetica", 15),
                      command=speech_to_text)
button_stt_1.grid(row=3, column=2, pady=10, padx=5)
